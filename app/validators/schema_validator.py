...
class SchemaValidator(ValidatorInterface):
    def check_validation(
        self, df, current_schema, context: ProcessingContext = None
    ) -> ValidatorInterface:
        schema = context.config["schema"]
        logger.debug(schema)
        logger.info(f"schema validation started")
        for col, expected_type in schema.items():
            ...
            if expected_type == "string" and not pd.api.types.is_string_dtype(df[col]):
                ...
                self.go_ahead = False
        logger.info("schema validation ended")
        return self
...
class ColumnValidator(ValidatorInterface):
    def check_validation(
        ...
        if file_columns != set(schema.keys()):
            self.error.append(
                f"expected schema {set(schema.keys())} and data schema {file_columns} doesn't match "
            )
            self.go_ahead = False
